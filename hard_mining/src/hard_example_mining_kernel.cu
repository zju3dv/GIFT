...
__global__ void sq_semi_hard_example_mining_kernel(
    float* feats,           // b,h,w,f
    float* feats_dist_sq,   // b,n
    float* feats_ref,       // b,n,f
    float* gt_loc,          // b,n,2
    int* hard_idxs,         // b,n,3
    int interval,           // 16 or 32
...
)
{
    ...
    float gtx=gt_loc[bi*n*2+ni*2];
    float gty=gt_loc[bi*n*2+ni*2+1];
    float dist_pos_sq=feats_dist_sq[bi*n+ni];
...
)
{
    ...
    assert(feats_ref.size(0)==b);
    assert(feats_ref.size(2)==f);
    assert(feats_dist_sq.size(0)==b);
    assert(feats_dist_sq.size(1)==n);
    assert(gt_loc.size(0)==b);
    assert(gt_loc.size(1)==n);
    assert(gt_loc.size(2)==2);
    assert(hard_idxs.size(0)==b);
    ...
    if(cross_batch)
        ...
        sq_semi_hard_example_mining_kernel<<<bdim,tdim>>>(
            feats.data<float>(),
            feats_dist_sq.data<float>(),
            feats_ref.data<float>(),
            gt_loc.data<float>(),
            hard_idxs.data<int>(),
            interval,thresh_sq,margin,
